...
namespace nilnul.num._quotient_._TEST_.eg_.fractile.convergent_.seriesly
{
	[TestClass]
		...
		void v(IEnumerable<int> x, DenomNonnilI expected) {
			Assert.IsTrue(
				nilnul.num.quotient.Eq2.Singleton.Equals(
					num.integer.str_.started_.nonhead8positive.fractile.convergents_._SerieslyX._Last_0fractile(
						x
					)
					,
					expected
